...
## Workflow Per Procedure (INCREMENTAL - ONE AT A TIME)
...
### Step 4: Update TODO Immediately ⚠️ CRITICAL
**Update tracker after EACH document - never batch!**

```
Edit: .DATABASE_STORED_PROCEDURES_TODO.md
1. Change: - [ ] PROCEDURE_NAME → - [x] PROCEDURE_NAME
2. Update: - **Completion Progress**: X/296 (Y.Y%) → (X+1)/296 (new%)
```

**Why Incremental**:
- ✅ Zero progress loss if session interrupted
- ✅ Always accurate current status
- ✅ Seamless multi-session continuation

### Step 5: Confirm & Continue
- Report: "✅ [Procedure Name] complete (N/296)"
- Move to next procedure immediately
- **NO questions - just keep working until module complete**
- **Only ask user when ALL procedures in a module are done**
...
### Session Startup (3 commands)
```bash
# 1. Read TODO to find next procedure
Read: Documents/Database/.DATABASE_STORED_PROCEDURES_TODO.md

# 2. Find DataService file
Glob: **/*DataService.cs

# 3. Read the service file
Read: [path_from_glob_result]
```

### Update Tracker (2 edits per procedure)
```bash
# Edit 1: Mark procedure complete
Edit: .DATABASE_STORED_PROCEDURES_TODO.md
old_string: "- [ ] PROCEDURE_NAME"
new_string: "- [x] PROCEDURE_NAME"

# Edit 2: Update progress counter
Edit: .DATABASE_STORED_PROCEDURES_TODO.md
old_string: "- **Completion Progress**: X/296 (Y.Y%)"
new_string: "- **Completion Progress**: (X+1)/296 (Z.Z%)"
```

### Progress Calculation
```
New percentage = ((X + 1) / 296) × 100, rounded to 1 decimal
Example: (27 / 296) × 100 = 9.1%
```
...
**Status**: ACTIVE
**Purpose**: Eliminate unnecessary discussion, maximize productivity, track progress incrementally
